...
async def debug_trigger_zone_email(
    ...
    """
    Debug endpoint — manually triggers a zone alert email for company 1 / GBP/USD.
    Deletes the most recent zone_change_log entry first so the change-detection
    logic fires even if the zone hasn't actually changed.
    ...
    company_id = 1
    pair = "GBP/USD"
    current_zone = "opportunistic"

    try:
        # 1. Delete most recent log entry for this company+pair so detection fires
        db.execute(_text("""
            DELETE FROM zone_change_log
            WHERE id = (
                SELECT id FROM zone_change_log
                WHERE company_id = :cid AND currency_pair = :pair
                ORDER BY created_at DESC LIMIT 1
            )
        """), {"cid": company_id, "pair": pair})
        db.commit()
        print(f"[debug-zone-email] deleted last log entry for {pair}")

        # 2. Fetch company alert email
        row = db.execute(_text("SELECT alert_email FROM companies WHERE id = :cid"), {"cid": company_id}).fetchone()
        alert_email = row._mapping["alert_email"] if row else None
        print(f"[debug-zone-email] alert_email={alert_email}")

        if not alert_email:
            return {"error": "No alert_email configured for company 1"}

        resend_key = os.getenv("RESEND_API_KEY")
        print(f"[debug-zone-email] resend_key present={bool(resend_key)}")
        if not resend_key:
            return {"error": "RESEND_API_KEY not set"}

        # 3. Log the zone change
        db.execute(_text("""
            INSERT INTO zone_change_log
                (company_id, currency_pair, previous_zone, new_zone,
                 trigger_type, spot_rate, budget_rate, pct_move, created_at)
            VALUES (:cid, :pair, 'base', :new, 'debug', 0, 0, 0, NOW())
        """), {"cid": company_id, "pair": pair, "new": current_zone})
        db.commit()
        print(f"[debug-zone-email] logged zone change base → {current_zone}")

        # 4. Send email via Resend
        zone_label = current_zone.upper()
        body_html = (
            f"<p><strong>[DEBUG TEST]</strong> {pair} has moved into the "
            f"<strong>{zone_label}</strong> zone.</p>"
            f"<p>This is a test email triggered manually via the debug endpoint.</p>"
        ...
        try:
            ...
            print(f"[zone-email] sent to {alert_email} for {pair} - {current_zone} | status={resp.status_code} body={resp.text}")
            return {"status": "triggered", "to": alert_email, "resend_status": resp.status_code, "resend_body": resp.text}
        except Exception as e:
            print(f"[zone-email] FAILED for {pair}: {e}")
            return {"error": str(e)}

    except Exception as e:
        logger.error(f"[debug-zone-email] outer exception: {e}")
        return {"error": str(e)}
